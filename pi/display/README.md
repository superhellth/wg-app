...
LCD: PCF8574 I2C backpack at `0x3f` on bus 1 (`i2c-1`).

## Setup on the Pi

1. Enable I2C: `sudo raspi-config` → Interface Options → I2C → enable
   (or add `dtparam=i2c_arm=on` to `/boot/firmware/config.txt`, then reboot).
2. Confirm the address: `i2cdetect -y 1` should show `3f`. If it shows a
   different address, set `LCD_ADDRESS` on the `display` service in compose.
